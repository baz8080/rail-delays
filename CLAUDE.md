...
| Nothing dispatches this site's build. `lifts-data` asks for the lift site's and only that one, so the two late crons are the whole cadence here until a second step there names this workflow and `SITE_BUILD_TOKEN` is widened to this repository. Both are owner actions in another repository | `notes/site.md` § How this gets published |
...
| The day bar drops `--fair`: at 22px it read as the same colour as `--good`, the way esb's own day cells found. Three bands past zero (1-3, 4-9, 10 or more) replace four, cut where 41 days of corpus splits evenly; the top band stays open-ended because nothing has reached 30. The no-data caption names the collector instead of repeating "data" next to "nothing listed" | `notes/site.md` § The day bar's colours and words, reviewed |
| "Cell" never reaches a reader on any of the four sites; this page's one visible use became "box". The header's sub line stopped repeating the row cards, the "notice going up" disclosure took lifts' pairing but not its claim that a disruption "ends" (this site tracks no end), and "Collected data" / "Lift outages" left the footer | `notes/site.md` § More words read back, from a live review |
| Hovering a day box did nothing on every build this site ever shipped: `bindDayCaption()` was inlined but never called, and the bar had no `.row`/`.card` ancestor for its handler to find the day's `.daycap` in. Both fixed; `TheHoverCaptionActuallyFires` pins the call and the markup so it cannot regress silently | `notes/site.md` § Hovering did nothing |
| The day caption dropped its family breakdown for a plain count: it read as a sentence competing with the row below it, and could sum to more than the total by design (a fault and its own knock-on both count), which reads as a mistake rather than the documented behaviour it is. `FAMILY_LABEL` left `render.py` with it | `notes/site.md` § The day caption stopped naming families |
| The disruption list pages client-side past `PAGE_SIZE` (10) rather than gaining a server-side page or a count floor: every row still ships in the month's own budget, `pageDelays()` only changes what is on screen. Not corpus-derived, a plain readability choice | `notes/site.md` § The disruption list paged client-side |
| Three chips shortened - "Knock-on delay", "Passenger issue", "Incident on the line" - each because the dropped words repeated something already sitting next to the chip in the row or added nothing the 41 `incident` causes ever actually state | `notes/site.md` § Two chips got shorter |
| The footer's four disclosures are one: "How to read a disruption" plus the source-code line. "How a cause is read", "What this measures", "What this cannot tell you" and the "notice going up" paragraph all restated ground `notes/site.md` already covers in more depth, and left with them: the page's last link to `baz8080/lifts`, and `render.py`'s now-unread `START` marker | `notes/site.md` § The footer lost the rest of its disclosures |
| "A report of a vulnerable person near/on the line" (15 clauses, found 2026-09-17) is read as `INCIDENT`, the same refusal-to-decode as "a tragic incident ... emergency services". "A points issue" (2 clauses) was left unread: real and specific, but two clauses is thinner evidence than any category here was built on | `notes/cause-reading.md` § A euphemism found after the count above was taken |
| The banner prints `Updated 44 minutes ago` rather than `Data to <stamp>`, from statusui's `freshness()`. The old rejection was wrong that a static page can only print a build-time age; the 15 KB it would have cost was real, and was fixed by splitting `freshness.js` out upstream behind `<!--UI-JS-FRESH-->`. The stamp stays in the HTML for a reader with no JS, the stale sentence is the shared one, and the browser gets `STALE_TO_READER` (24h) rather than `STALE_AFTER` (10h) because nothing dispatches this build | `notes/site.md` § The banner prints an age |
| "Newest first, by when Irish Rail first listed the notice" is "Newest first" - every row underneath already carries the date. The capacity count went from twenty-six words to six: "142 trains had reduced capacity, but they are not counted as a disruption" (fixed from an initial "delay" - the one word this page's whole subject avoids) | `notes/site.md` § "Newest first" and the capacity count shortened |
| The origins panel's intro, the tiles' labels and the legend all shortened on the same pass: "named something that went wrong" to "named a cause", "named no cause at all" to "named no cause", every band in the legend lost "listed" ("nothing", "1 to 3", "4 to 9", "10 or more") - the hover caption's own "nothing listed" is untouched | `notes/site.md` § Four more trims, and one word fixed |
| The `.when` minutes badge folded into the row's `.sum` sentence. esb pulled the same span for the same reason and lifts' one is a derived figure its notices never state, so the sibling precedent the question was raised on points the other way. The figure itself stays: on 20 of 346 disruptions the quoted wording eased or dropped it, and that is the only place worst-rather-than-newest is visible | `notes/site.md` § The minutes badge folded into the row's sentence |
