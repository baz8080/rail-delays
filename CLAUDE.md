...
| The disruption list pages client-side past `PAGE_SIZE` (20) rather than gaining a server-side page or a count floor: every row still ships in the month's own budget, `pageDelays()` only changes what is on screen. Not corpus-derived, a plain readability choice | `notes/site.md` § The disruption list paged client-side |
